...
public class testViewReservationByHostId {
    ...
    @Test
    public void testAllReservationsByHostId() throws Exception {
        ...
        Mockito.when(reservationService.allReservationsByHostId()).thenReturn(reservationList);
        ...
        mockMvc.perform(get("/api/party/reservation/allbyhost").header("Authorization", "Bearer " + token)).andExpect(status().isOk()).andExpect(jsonPath("$", Matchers.hasSize(1)))
                ...
                .andExpect(jsonPath("$[0].status", Matchers.equalTo("P")));
    }
